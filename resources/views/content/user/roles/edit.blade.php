...
<h4 class="fw-bold py-3 mb-1">
  <span class="text-muted fw-light">Roles /</span> Role Edit
</h4>
...
<div class="row">
  <div class="col-12">
    ...
    <div class="card">
      <h5 class="card-header crud">Edit Role Info</h5>

      <div class="card-body">
        {{ html()->model($role)->form('PATCH')->route('roles.update', $role->id)->class('row g-3')->id('validateForm')->open() }}
              ...
              <div class="row">
                ...
                <div class="col-md-3">
                  <div class="form-check form-check-primary">
                    {{ html()->checkbox('permissions[]', null, $permission->id)->class('form-check-input
                    check')->id('checkboxPermission'.$permission->id)->checked(in_array($permission->id, $role->permissions->pluck('id', 'id')->all()) ? 1 : 0) }}
                    {{ html()->label($permission->display_name,
                    'checkboxPermission'.$permission->id)->class('form-check-label text-primary') }}
                  </div>
                </div>
                @endforeach
              </div>
        ...
        <div class="col-12">
          ...
          auth('web')->user()->hasPermission(['user-roles-read']))
          <a href="{{ route('roles.index') }}" type="button" class="btn btn-label-danger waves-effect">Cancel</a>
          @endif
        </div>
        {{ html()->form()->close() }}
      </div>
