        ...
        <tbody>
          ...
          <tr class="@if($role->name == 'admin') disabled @endif">
            <td class="text-center">
              ...
              auth('admin')->user()->hasPermission(['admin-roles-update']))
              <a class="btn btn-sm btn-icon btn-outline-primary waves-effect"
                href="{{ route('admin.roles.edit', $role->id) }}" data-bs-toggle="tooltip" data-bs-placement="top"
              ...
              @if(auth('admin')->user()->hasRole('admin') ||
              auth('admin')->user()->hasPermission(['admin-roles-delete']))
              ...
              {{ html()->form('DELETE')->route('admin.roles.destroy',$role->id)->id('deleteForm'.$role->id)->open() }}
              {{ html()->form()->close() }}
              @endif
